...
function displayInteractions() {
    ...
    if (storedData) {
        ...
        if (currentInteractions) {
            const interactions = currentInteractions.interactions.sort((a, b) => new Date(b.datetime) - new Date(a.datetime));
            const interactionList = interactions.map((interaction) => {
                let cardContent = '';
                if (interaction.reason) cardContent += `<h2 class="text-2xl font-bold mb-2">${interaction.reason}</h2>`;
                if (interaction.datetime) cardContent += `<p class="text-gray-600">Date: ${new Date(interaction.datetime).toLocaleString()}</p>`;
                if (interaction.cn) cardContent += `<p class="text-gray-600">Calling Number: ${interaction.cn}</p>`;
                if (interaction.in) cardContent += `<p class="text-gray-600">Incident Number: ${interaction.in}</p>`;
                if (interaction.name) cardContent += `<p class="text-gray-600">Name: ${interaction.name}</p>`;
                if (interaction.findings) cardContent += `<p class="text-gray-600">Findings: ${interaction.findings}</p>`;
                if (interaction.resolution) cardContent += `<p class="text-gray-600">Resolution: ${interaction.resolution}</p>`;
                if (cardContent) {
                    return `
                        <div class="bg-white p-4 rounded shadow-md mb-4">
                            ${cardContent}
                            <div class="flex justify-end mt-4">
                                <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mx-2" onclick="copyInteraction(${interaction.id})">Copy</button>
                                <button class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded mx-2" onclick="updateInteraction(${interaction.id})">Update</button>
...
//         const currentInteractions = data.find((item) => item.date === currentDate);
//         if (currentInteractions) {
//             const interactionToCopy = currentInteractions.interactions.find((interaction) => interaction.id === id);
//             if (interactionToCopy) {
//                 // Create a text string with the interaction details
//                 const textToCopy = `Interaction ${interactionToCopy.id}\n`;
//                 textToCopy += `Date: ${new Date(interactionToCopy.datetime).toLocaleString()}\n`;
//                 textToCopy += `Calling Number: ${interactionToCopy.cn}\n`;
//                 textToCopy += `Incident Number: ${interactionToCopy.in}\n`;
//                 textToCopy += `Name: ${interactionToCopy.name}\n`;
//                 textToCopy += `Reason for Calling: ${interactionToCopy.reason}\n`;
//                 textToCopy += `Findings: ${interactionToCopy.findings}\n`;
//                 textToCopy += `Resolution: ${interactionToCopy.resolution}\n`;
